...
export class CDPManager extends EventEmitter {
    sessions = new Map();
    pageToSessionMap = new WeakMap();
    contextPages = new WeakMap();
    constructor() {
        super();
    }
    /**
     ...
     */
    async sendCommand(page, method, params) {
     ...
     */
    async closeAll() {
        const closePromises = Array.from(this.sessions.values()).map(session => session.close().catch(err => console.error('Error closing session:', err)));
